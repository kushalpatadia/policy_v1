  ...
  <tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
      ...
      <td class="hidden-xs hidden-sm text-center"><?php if(!empty($row['status']) && $row['status']==1){ ?>
        <a href="<?php echo  $this->config->item('admin_base_url').$viewname; ?>/unpublish_record/<?php echo  $row['id'] ?>"> <img title="Unpublish Record" src="<?php echo $this->config->item('image_path');?>publish_icon.png" width="22"  /> </a>
        <? }else{ ?>
