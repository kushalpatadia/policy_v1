          ...
          <div class="portlet-content">
            <div class="table-responsive">
              <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
                  ...
                  <div class="col-sm-11">
                    <div class="dataTables_filter" id="DataTables_Table_0_filter">
                      <label>
                        ...
                        <input type="text" name="searchtext" id="searchtext" aria-controls="DataTables_Table_0" placeholder="Search..." title="Search <?=$this->lang->line('admin_short')?>" value="<?=!empty($searchtext)?$searchtext:''?>">
                        <button onclick="contact_search('changesearch')" class="btn btn-success howler"  title="Search Admin">Search </button>
                        <button class="btn btn-success howler flt" title="View All <?=$this->lang->line('admin_short')?>" onclick="clearfilter_contact()" title="Reset">view All</button>
                      </label>
                    </div>
                  </div>
                </div>
                <div class="row dt-rt">
                  <div class="col-sm-6">
                    <button class="btn btn-danger howler" data-type="danger" title="Delete <?=$this->lang->line('admin_short')?>" onclick="deletepopup1('0');">Delete
                    <?=$this->lang->line('admin_short')?>
                    </button>
                  </div>
                  <div class="col-sm-6"> <a class="btn  pull-right btn-success howler" title="Add New <?=$this->lang->line('admin_short')?>" href="<?=base_url('admin/'.$viewname.'/add_record');?>">Add New <?=$this->lang->line('admin_short')?></a> </div>
                  ...
                  <?php
			            ...
			            ?>
 					    <p>
 						 <div id="div_msg" class="message" style="display:none; text-align:center;"> <?php echo '<label class="error">'.$this->message_session['msg'].'</label>';
						$newdata = array('msg'  => '');
	...
	function contact_search(allflag)
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>admin/admin_management/"+uri_segment,
				data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/admin_management/"+data,
					data: {
