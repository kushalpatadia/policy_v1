          ...
          <div class="portlet-content">
            <div class="col-sm-8">
              <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?= $this->config->item('admin_base_url')?><?php echo $path?>" >
                <div class="form-group">
                  ...
                  <input id="title" name="title" class="form-control parsley-validated" type="text" data-required="required" value="<?= !empty($editRecord[0]['title'])?$editRecord[0]['title']:'';?>">
                </div>
                
                <div class="form-group">
            <label for="firstname" class="col-lg-5 col-md-12 control-label">Slider Image <span>:</span> </label>
			...
			if($this->uri->segments[2] =='add_record'){ ?>
                ...
                <?php if(!empty($editRecord[0]['image']))
                        {?>
                <img id="uploadPreview1" src="<?=base_url($this->config->item('slider_small_img_path')).'/'.(!empty($editRecord[0]['image'])?$editRecord[0]['image']:'');?>"  width="100"  height="100" />
                <? } else{?>
